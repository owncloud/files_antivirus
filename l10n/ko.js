OC.L10N.register(
    "files_antivirus",
    {
    "Clean" : "감염되지 않음",
    "Infected" : "감염됨",
    "Unchecked" : "확인되지 않음",
    "Scanner exit status" : "검사 도구 종료 결과",
    "Scanner output" : "검사 도구 출력",
    "Saving..." : "저장 중...",
    "Test scan was unsuccessful. Please recheck antivirus settings." : "테스트 검사가 실패했습니다. 안티바이러스 설정을 다시 확인하십시오.",
    "<strong>Infected file</strong> has been <strong>found</strong>" : "<strong>감염된 파일</strong>을 <strong>찾았습니다</strong>",
    "File %s is infected with %s" : "파일 %s이(가) %s에 감염되었습니다",
    "It is going to be deleted" : "해당 항목이 삭제됩니다",
    "Antivirus" : "안티바이러스",
    "Virus %s is detected in the file. Upload cannot be completed." : "파일에서 %s 바이러스가 탐지되었습니다. 업로드할 수 없습니다.",
    "Saved" : "저장됨",
    "Virus detected! Can't upload the file %s" : "바이러스가 감지되었습니다! 파일 %s을(를) 업로드할 수 없습니다",
    "Either the ownCloud antivirus app is misconfigured or the external antivirus service is not accessible. Please reach out to your system administrator!" : "ownCloud 안티바이러스 앱이 잘못 설정되었거나 외부 안티바이러스 서비스에 접근할 수 없습니다. 시스템 관리자에게 연락해주세요!",
    "Malware detected" : "멀웨어 감지됨",
    "A malware or virus was detected, your upload was denied. In doubt or for details please contact your system administrator." : "멀웨어 또는 바이러스가 감지되어 업로드가 거절되었습니다. 시스템 관리자에게 연락하여 자세히 알아보세요.",
    "Greetings {user}," : "{user}님, 환영합니다",
    "This email is a notification from {host}. Please, do not reply." : "이 이메일은 {host}에서 보낸 알림입니다. 답장하지 마십시오.",
    "File uploaded: {file}" : "파일 업로드됨: {file}",
    "Antivirus Configuration" : "안티바이러스 설정",
    "Mode" : "모드",
    "Executable" : "실행 파일",
    "Socket" : "소켓",
    "Clamav Socket" : "Clamav 소켓",
    "Host" : "호스트",
    "Hostname or IP address of Antivirus Host" : "안티바이러스 호스트의 이름이나 IP 주소",
    "Port" : "포트",
    "Port number of Antivirus Host, 1-65535" : "안티바이러스 호스트의 포트 번호, 1-65535",
    "ICAP request service. Possible values: \"avscan\" for clamav or \"req\" for Kaspersky ScanEngine" : "ICAP 요청 서비스. 가능한 값: clamav는 \"avscan\", Kaspersky ScanEngine은 \"req\"",
    "ICAP response header holding the virus information. Possible values: X-Virus-ID or X-Infection-Found" : "바이러스 정보를 담고 있는 ICAP 응답 헤더입니다. 가능한 값: X-Virus-ID 또는 X-Infection-Found",
    "ICAP request service. Fortinet uses \"respmod\"" : "ICAP 요청 서비스. Fortinet은 \"respmod\"를 사용합니다.",
    "ICAP response header. Fortinet sends X-Virus-ID" : "ICAP 응답 헤더. Fortinet이 X-Virus-ID를 보냈습니다.",
    "ICAP request service. McAfee Webgateway uses \"respmod\"" : "ICAP 요청 서비스. McAfee Webgateway는 \"respmod\"를 사용합니다.",
    "ICAP response header. McAfee Webgateway sends X-Virus-Name" : "ICAP 응답 헤더. McAfee Webgateway가 X-Virus-ID를 보냈습니다.",
    "Path to clamscan" : "clamscan 경로",
    "Extra command line options (comma-separated)" : "추가 명령행 옵션(쉼표로 구분)",
    "Stream Length" : "스트림 길이",
    "ClamAV StreamMaxLength value in bytes" : "바이트 단위 ClamAV StreamMaxLength 값",
    "bytes" : "바이트",
    "File size limit, -1 means no limit" : "파일 크기 제한, -1은 제한 없음",
    "File size limit in bytes, -1 means no limit" : "바이트 단위 파일 크기 제한, -1은 무제한",
    "Background scanning" : "백그라운드 스캐닝",
    "Enabled" : "활성",
    "Disabled" : "비활성화됨",
    "When infected files were found during a background scan" : "백그라운드 검사에서 감염된 파일을 발견했을 때 행동",
    "Only log" : "기록만",
    "Delete file" : "파일 삭제",
    "Save" : "저장",
    "Advanced" : "고급",
    "Rules" : "규칙",
    "Clear All" : "모두 지우기",
    "Reset to defaults" : "기본값 복원",
    "Match by" : "다음으로 일치",
    "Scanner exit status or signature to search" : "검사 도구 종료 결과 및 검색할 패턴",
    "Description" : "종류",
    "Mark as" : "다음으로 표시",
    "Add a rule" : "규칙 추가"
},
"nplurals=1; plural=0;");
